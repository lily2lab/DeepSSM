...
import os.path
import numpy as np
from nets import DeepSSM_h36m as  DeepSSM
...
def main(argv=None):
	...
	for itr in range(1, FLAGS.max_iterations + 1):
		...
		if itr % test_interval == 0:
			...
			for s in str1:
				...
				if  not tf.gfile.Exists(path):
					os.mkdir(path)
				span=(FLAGS.batch_size*FLAGS.n_gpu)/8
				for ik in range(8):
		...
		if itr % snapshot_interval == 0 and min(errlist) < min_err:
			model.save(itr)
			min_err = min(errlist)
