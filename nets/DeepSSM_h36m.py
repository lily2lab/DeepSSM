...
def DeepSSM(images,keep_prob, seq_length, input_length, stacklength1, stacklength2, num_hidden,filter_size):
	with tf.variable_scope('DCCM', reuse=False):
		print 'DCCM'
		...
		for s in range(2):
			inputs = inputsall[s]
			inputs_db = []
			with tf.variable_scope('stream', reuse = bool(general_diff1)):
				for c in range(3):
					inputs_db1 = inputs[:,:,:,c]
		...
		for i in range(seq_length-input_length):
			if i%2 == 0:
				...
				# fusion feature
				#print('i,ii',i,i//2)
				with tf.variable_scope('hier_fusing_info'+str(i), reuse=False):
